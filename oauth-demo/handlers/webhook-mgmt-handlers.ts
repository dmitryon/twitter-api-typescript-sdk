...
export const listWebhooks = async (req: Request, res: Response) => {
  try {
    const client = await appBearerClient(req.params.appId);
...
export const createWebhook = async (req: Request, res: Response) => {
  try {
    ...
    const client = await appBearerClient(appId);
...
export const createSubscription = async (req: Request, res: Response) => {
  try {
    ...
    const resolved = await resolveAuth(integrationId, effectiveAuthType);
    if (!resolved) {
      res.status(400).json({ error: "Integration not found or invalid auth" });
      return;
    }

    const result = await resolved.client.accountactivity.createAccountActivitySubscription(req.params.webhookId, {});
